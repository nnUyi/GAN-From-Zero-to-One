...
class CGAN:
    ...
    def discriminator(self, input_x, input_labels, scope_name='discriminator', is_training=True, reuse=False):
        ...
        with tf.variable_scope(scope_name) as scope:
            ...
            with slim.arg_scope([slim.fully_connected],
                ...
                flat_c = tf.concat([flat, input_labels], 1)
                # 2048
                fc0 = l_relu(slim.fully_connected(flat_c, 2048, scope='d_fc0'))
                # 1024
    ...
    def build_model(self):
        ...
        # image labels
        self.input_labels = tf.placeholder(dtype=tf.float32,
                                           shape=[self.batchsize, self.num_class],
                                           name='input_labels')

        
        # concatenate condition in the tail of noise_z
        self.condition_z = tf.concat([self.z, self.input_labels], 1)

        _, self.fake_image = self.generator(self.condition_z, is_training=True, reuse=False)
        _, self.sample = self.generator(self.condition_z, is_training=False, reuse=True)

        self.real_unit = self.discriminator(self.input_x, self.input_labels, is_training=True, reuse=False)
        self.fake_unit = self.discriminator(self.fake_image, self.input_labels, is_training=True, reuse=True)
    ...
    def train_model(self):
        ...
        for epoch in range(self.config.epoch):
            ...
            for ite in tqdm(range(50000/self.batchsize)):
                ...
                # optimize generator
                _, g_loss = self.sess.run([self.g_optim, self.g_loss], feed_dict={self.z:noise_z, self.input_labels:input_labels})
                # optimize generator
                _, g_loss = self.sess.run([self.g_optim, self.g_loss], feed_dict={self.z:noise_z, self.input_labels:input_labels})

                # sample image during training phase        
                if np.mod(ite, 100)==0:
                    sample = self.sample.eval({self.z:noise_z, self.input_labels:sample_labels})
